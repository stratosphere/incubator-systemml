...
public class RandFLInstruction extends UnaryFLInstruction {
    ...
    private void generateRandData(FlinkExecutionContext flec) throws DMLRuntimeException {
        ...
        //b) file-based seed rdd construction (for robustness wrt large number of blocks)
            ...
            //create seeds dataset
            seedsData = flec.getFlinkContext().readCsvFile(path).types(Long.class, Long.class, Long.class, Long.class).map(new MapFunction<Tuple4<Long, Long, Long, Long>, Tuple3<MatrixIndexes, Long, Long>>() {
                @Override
                public Tuple3<MatrixIndexes, Long, Long> map(Tuple4<Long, Long, Long, Long> value) throws Exception {
                    return new Tuple3<MatrixIndexes, Long, Long>(new MatrixIndexes(value.f0, value.f1), value.f2, value.f3);
                }
            });
        }

        // step 4: execute rand instruction over seed input
        DataSet<Tuple2<MatrixIndexes, MatrixBlock>> out = seedsData.map(new MapFunction<Tuple3<MatrixIndexes, Long, Long>, Tuple2<MatrixIndexes, MatrixBlock>>() {
            @Override
            public Tuple2<MatrixIndexes, MatrixBlock> map(Tuple3<MatrixIndexes, Long, Long> value) throws Exception {
